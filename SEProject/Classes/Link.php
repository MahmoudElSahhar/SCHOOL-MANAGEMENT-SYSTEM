...
class Link{
    ...
    public static function add($l){
        ...
        $values = array($l->ID, $l->physicalAddress, $l->friendlyName);
        $db = Database::getInstance();
        $db->insert("link", $fields, $values);
    }

    public static function update($ly){
        $fields = array("ID","PhysicalAddress","FriendlyName","LastUpdated");
        $values = array($l->ID, $l->physicalAddress, $l->friendlyName, $l->lastUpdated);
        $db = Database::getInstance();
